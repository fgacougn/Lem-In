...
int graph_simplify(t_graphe_racine *racine)
{
    ...
    {
        if(racine->all[i]->is_peculiar == PECULIAR_NO &&  racine->all[i]->has_a_way == FALSE && arrete_len(racine->all[i]->arretes) == 2)
        {
            // ft_printf("Pop! %d\n", i);
            ...
            while(arrete_len(start->arretes) == 2 && start->is_peculiar == PECULIAR_NO)
                next_arrete(&start, &tempstart);
            while(arrete_len(end->arretes) == 2 && end->is_peculiar == PECULIAR_NO)
